...
read url
echo -n "Certificate path (leave blank if not using SSL): "
read cert
echo -n "Username: "
read username
echo -n "Password: "
...
# load index template
if curl -f -XPUT "$url/_index_template/web-logs" --key $cert -k -u $username:$password -H 'Content-Type: application/json' -d "@web-logs-template.json"
...
if curl -f -XPUT "$url/_ingest/pipeline/web-logs" --key $cert -k -u $username:$password -H 'Content-Type: application/json' -d "@web-logs-pipeline.json"
